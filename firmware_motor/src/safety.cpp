...
// Error codes (local, not in protocol)
#define ERR_NONE        0
#define ERR_TIMEOUT     1
#define ERR_STALL_LEFT  2
#define ERR_STALL_RIGHT 3
...
bool safety_check(uint8_t pwm_a, uint8_t pwm_b, float rpm_a, float rpm_b) {
    ...
    if (pwm_a > SAFETY_STALL_PWM_THRESHOLD && abs((int)rpm_a) < SAFETY_STALL_RPM_THRESHOLD) {
        if (stall_start_a == 0) stall_start_a = now;
    ...
    if (pwm_b > SAFETY_STALL_PWM_THRESHOLD && abs((int)rpm_b) < SAFETY_STALL_RPM_THRESHOLD) {
        if (stall_start_b == 0) stall_start_b = now;
...
uint8_t safety_get_error() { return current_error; }
...
void safety_feed_watchdog() { wdt_reset(); }

int16_t safety_ramp_limit(int16_t current, int16_t desired) {
    int16_t diff = desired - current;
    if (diff > SAFETY_RAMP_MAX_RPM_PER_CYCLE) return current + SAFETY_RAMP_MAX_RPM_PER_CYCLE;
    if (diff < -SAFETY_RAMP_MAX_RPM_PER_CYCLE) return current - SAFETY_RAMP_MAX_RPM_PER_CYCLE;
    return desired;
}
